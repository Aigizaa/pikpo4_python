...
class DataProcessor(ABC):
    ...
    def run(self) -> None:
        ...
        # cleaned_result = self.remove_col_by_name(self._dataset, ["1990"])
        cleaned_result = self.filter_year(self._dataset, 2000)
        cleaned_result = self.sort_data_by_col(cleaned_result, "country", True)
        # cleaned_result = self.get_query(cleaned_result, 'country == "Italy"')
        # cleaned_result = self.get_query(cleaned_result, "country == 'Yemen'")
        cleaned_result = cleaned_result.drop(cleaned_result[cleaned_result['country'] == "Cote d'Ivoire"].index)
    ...
    def filter_year(self, df: pandas.DataFrame, year) -> pandas.DataFrame:
        new = pandas.DataFrame.head(self._dataset, 0)
        res = []
        heads = new.drop("country", axis=1)
        ...
        return self.remove_col_by_name(self._dataset, res)
